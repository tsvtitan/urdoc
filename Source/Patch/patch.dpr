...
uses
  Windows,
  SysUtils,
  BisLoaderInfo in 'U:\source\loader.exe\BisLoaderInfo.pas';
    ...
    if ParamCount>1 then begin
      ...
      if AnsiSameText(s1,SwitchMessage) then begin
        if ExitCode>0 then begin
          MessageBox(0,PChar('Обновление прошло неудачно.'),PChar('Ошибка'),MB_OK+MB_ICONERROR);
        end else begin
          MessageBox(0,PChar('Обновление прошло успешно.'),PChar('Информация'),MB_OK+MB_ICONINFORMATION);
        end;
      end;
    end;
  end;
