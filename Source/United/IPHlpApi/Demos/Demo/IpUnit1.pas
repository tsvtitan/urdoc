    ...
    for i := 0 to pTcpTable^.dwNumEntries do
      ...
      begin
        Add(Format(' %15s %5d %15s %5d %5d', [IpAddressToString(dwLocalAddr),
          htons(dwLocalPort), IpAddressToString(dwRemoteAddr), htons(dwRemotePort), dwState]));
      end;
  finally
   Freemem(pTcpTable);
  end;
    ...
    try
     ...
     for i := 0 to pUdpTable^.dwNumEntries do
       with pUdpTable^.table[i] do
         Memo1.Lines.Add(Format(' %15s %5d',[IpAddressToString(dwLocalAddr) ,htons(dwLocalPort)]));
    finally
      Freemem(pUdpTable);
    end;
end;
