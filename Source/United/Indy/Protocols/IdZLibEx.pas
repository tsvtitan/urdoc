...
{
{   Rev 1.5    3/10/2005 11:55:24 PM  JPMugaas
{ I had to expose some constants so things could compile.
}
{
{   Rev 1.4    3/10/2005 11:33:48 PM  JPMugaas
{ Updated to latest version of ZLibEx.  Thiere was a Cert Warning about ZLIB
{ 1.21 (http://www.kb.cert.org/vuls/id/238678).
}
{
...
procedure ZCompressStream2(inStream, outStream: TStream;
  level: Integer; windowBits, memLevel: Integer;
  strategy: Integer);  overload;
...
type
  EIdZLibError = class(EIdException)
  private
    FErrorCode: Integer;
  public
  ...
  EIdZCompressionError = class(EIdZLibError);
  EIdZDecompressionError = class(EIdZLibError);
...
begin
  ...
  if code < 0 then
  begin
    raise EIdZCompressionError.Create(code);
  end;
end;
...
begin
  ...
  if code < 0 then
  begin
    raise EIdZDecompressionError.Create(code);
  end;
end;
...
begin
  ...
  begin
    ...
    //this has to run, else memory leak
    ZDecompressCheck(inflateEnd(zstream));
  end;
end;
...
procedure ZCompressStream(inStream, outStream: TStream;
  level: TZCompressionLevel);
begin
  ZCompressStream(inStream,outStream,ZLevels[level]);
end;
...
begin
  ...
  ZCompressCheck(DeflateInit2(zstream,level,Z_DEFLATED,windowBits,
    memLevel,strategy));
...
begin
  ZCompressStream2(inStream,outStream,
    ZLevels[level],windowBits,memLevel,ZStrategies[strategy]);
end;
...
begin
  raise EIdZCompressionError.Create(SZInvalid);
end;
...
begin
  ...
  else raise EIdZCompressionError.Create(SZInvalid);
end;
...
begin
  raise EIdZDecompressionError.Create(SZInvalid);
end;
...
begin
  ...
  else raise EIdZDecompressionError.Create(SZInvalid);
...
constructor EIdZLibError.Create(code: Integer);
begin
