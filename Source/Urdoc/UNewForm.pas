...
//       SetFirstTabSheet;
       mr:=MessageDlg('Сохранить изменения в форме <'+Caption+'> ?',mtConfirmation,[mbYes,mbNo,mbCancel],-1);
{       but:=MessageBox(Application.Handle,pchar('Сохранить изменения в форме <'+Caption+'> ?'),
                      'Вопрос',MB_ICONQUESTION+MB_YESNO);}
       case mr of
         mrYes: begin
...
//           mr:=MessageDlg('Документ <'+GetDocName(LastDocId)+'> не будет сохранен. Продолжить?',mtWarning,[mbYes,mbNo],-1);
           mr:=MessageDlg('Сохранить изменения в документе <'+GetDocName(LastDocId)+'>?',mtConfirmation,[mbYes,mbNo,mbCancel],-1);
           case mr of
             mrYes: begin
               bibOk.OnClick(nil);
               CanClose:=false;
...
//  if not SetEmptyPadegControls then exit;
  if isCheckFieldsFill then
   if isEmptySelfControls(pnDesign,BreakF) then begin
    ShowError(Application.Handle,'Одно из полей ввода не заполнено.');
    exit;
   end;
  if cbCurReestr.ItemIndex=-1 then begin
   ShowError(Application.Handle,'Выберите реестр.');
   cbCurReestr.SetFocus;
   exit;
  end;
  if cbNumLic.ItemIndex=-1 then begin
   ShowError(Application.Handle,'Выберите лицензию.');
   cbNumLic.SetFocus;
   exit;
  end;
  if cmbMotion.ItemIndex=-1 then begin
   ShowError(Application.Handle,'Выберите нотариальное действие.');
   cmbMotion.SetFocus;
   exit;
  end;
  if Trim(cbFio.Text)='' then begin
   ShowError(Application.Handle,'Выберите или введите Фамилию И.О.');
   cbFio.SetFocus;
   exit;
  end;
  if Trim(edDocName.Text)='' then begin
   ShowError(Application.Handle,'Введите наименование документа.');
   edDocName.SetFocus;
   exit;
  end;
  if isCheckEmptySumm then
    if (Trim(edNumReestr.Text)<>'') and (SummEdit.Value=0.0) then begin
     ShowError(Application.Handle,'Сумма для активных не может быть равна 0.');
     SummEdit.SetFocus;
     exit;
    end;
  ...
  if Assigned(cbBlank) and Assigned(edBlank) and not VarIsNull(BlankNum) then begin
    if cbBlank.ItemIndex<>-1 then begin
      ...
      if Assigned(Obj) then begin
        ...
        if Flag then begin
          ShowError(Application.Handle,'Номер бланка не входит в диапазон серии.');
          edBlank.SetFocus;
          exit;
        end;
      end;
...
begin
  ...
  if Trim(edNumReestr.Text)<>'' then begin
    if isNumReestrAlready(Strtoint(edNumReestr.Text),LastTypeReestrID) then begin
      ShowError(Application.Handle,'Номер в реестре <'+edNumReestr.Text+'> уже существует.');
...
begin
  ...
  if Not isInsert then
   if not IsReestrIDFound then begin
    ShowError(Application.Handle,'Эта запись в реестре была удалена, обновите реестр.');
    close;
    exit;
   end;
...
begin
        ...
        // if not tb.Locate('reestr_id',LastReestrID,[loCaseInsensitive]) then exit;
         ...
         if not IsOtlogen then begin
          ...
          plusstr:=' по реестру № '+edNumReestr.Text;
         end else begin
         ...
         if Trim(tb.FieldByName('parent_id').AsString)='' then
          ...
          tb.FieldByName('tmpname').AsString:='Копия ('+
...
procedure TfmNewForm.DestroyHeaderAndCreateNew;
   ...
   begin
     ...
     bt.ShowHint:=true;
     bt.Caption:='<-';
     bt.Hint:='Следующий номер в реестре';
     ...
     lb.Top:=52;
     lb.Height:=26;
     lb.Caption:='Нотариальное действие:';
     lbMotion:=lb;
     ...
     ch.Caption:='Н/летние';
     ch.ShowHint:=true;
     ch.Hint:=' Договоры в которых стороной или'+#13+
              'правообладателями значатся'+#13+
              'несовершеннолетние';
     ch.TabOrder:=8;
     chbNoYear:=ch;
     ...
     ch.Top:=57;
     ch.Height:=17;
     ch.Caption:='Физ.недостатки';
     ch.ShowHint:=true;
     ch.Hint:='  Договоры за удостоверением которых'+#13+
              'обратились гр-не имеющие физические недостатки';
     ch.TabOrder:=9;
     chbDefect:=ch;
     ...
     lb.Top:=84;
     lb.Height:=13;
     lb.Caption:='Фамилия И.О.:';
     lbFio:=lb;

     cb:=TComboBox.Create(Self);
     cb.Parent:=sb;
     cb.ShowHint:=true;
     cb.Hint:='Фамилия Имя Отчество';
     cb.Name:='cbFio';
     cb.Left:=91;
     cb.Width:=265;
     cb.Top:=80;
     ...
     bt.ShowHint:=true;
     bt.Caption:='...';
     bt.Hint:='Выбрать наследственное дело';
     bt.Width:=21;
     ...
     lb.Top:=101;
     lb.Height:=26;
     lb.Caption:='Наименование документа:';
     lbDocName:=lb;
     ...
     ed.Name:='edDocName';
     ed.ShowHint:=true;
     ed.Hint:='Введите отличное от шаблона наименование документа';
     ed.MaxLength:=100;
     ed.Left:=91;
     ed.Width:=265;
     ed.Top:=105;
     ...
     dtp.Height:=21;
     dtp.TabOrder:=14;
     dtp.Hint:='Дата удостоверения документа';
     dtp.ShowHint:=true;
  ...
  try
    ...
    if fm.ShowModal=mrOk then begin
      if not fm.Mainqr.IsEmpty then begin
       ...
       edHereditaryDeal.Text:=OldHereditaryDealNum+' от '+DateToStr(OldHereditaryDealDate);
