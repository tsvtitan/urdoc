...
begin
  ...
  if isBackUpLast then begin
    if isQuestionBackup then begin
      Ret:=MessageDlg('Создать архив базы данных?',mtConfirmation,[mbYes,mbNo,mbCancel],-1);
      case Ret of
        mrYes: begin
          FlagBackUp:=true;
          Action:=caFree;
...
begin
  ...
  try
    ...
    if CheckDemo then begin
      mr:=MessageDlg('Вы достигли максимальное количество ограничений демонстрационной версии, закрыть программу?',mtWarning,[mbYes,mbNo],-1);
      if mr=mrYes then
        Close;
    end;
  finally
...
begin
  if not RunAnyFile(FileHelp) then
     showerror(Application.Handle,'Не удалось запустить файл помощи.');
end;
...
begin
  if isViewWhoCert then begin
    ...
    try
      ...
      fm.lbNotarius.Caption:='Нотариус или помощник';

      fm.meInfo.Lines.Text:='Выберите нотариуса или его помощника для автоматического создания удостоверительной надписи';
      if fm.ShowModal=mrOk then begin
