...
uses Windows, Classes, SysUtils, Variants, ShellApi, Graphics, Types;

type
  TSetOfChar=set of Char;

function CreateUniqueId: String;
function GetUniqueIdByIpAddress(const IpAddress: string): String;
function CheckUniqueIdByAdapter(const UniqueId: string): Boolean;
function GetUrlByPort(Security: Boolean; ServerPort: Integer; ServerName, Document: string): String;
...
var
  ...
  function CheckByAdapter: Boolean;
  var
    PIPAddr: PIpAddrString;
  begin
    Result:=false;
    if Assigned(PAdapter) then begin
      if Assigned(PAdapter.CurrentIpAddress) then begin
        if (PAdapter.CurrentIpAddress.IpAddress.S=IpAddress) then begin
          Result:=true;
          exit;
        end;
      end else begin
        ...
        repeat
          if Assigned(PIPAddr) then begin
            if (PIPAddr.IpAddress.S=IpAddress) then begin
              Result:=true;
              exit;
            end;
...
begin
  Result:='';
  if Security then
    Result:=Format(SUrlHttps,[ServerName,Document])
  else begin
    if ServerPort<>IdPORT_HTTP then
      Result:=Format(SUrlHttpPort,[ServerName,ServerPort,Document])
    else
      Result:=Format(SUrlHttp,[ServerName,Document]);
  end;
end;
