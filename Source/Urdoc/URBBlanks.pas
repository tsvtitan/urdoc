...
begin
  ...
  if Grid.Columns.Count<>3 then begin
   ...
   cl.Title.Caption:='Номер от';
   cl.Width:=100;

   cl:=Grid.Columns.Add;
   cl.FieldName:='num_to';
   cl.Title.Caption:='Номер по';
   cl.Width:=100;
...
begin
 ...
 try
  ...
  fm.edNumFrom.text:=Trim(MainQr.FieldByName('num_from').DisplayText);
  fm.edNumTo.text:=Trim(MainQr.FieldByName('num_to').DisplayText);
...
begin
  if Mainqr.RecordCount=0 then exit;
  mr:=MessageDlg('Удалить бланк?',mtConfirmation,[mbYes,mbNo],-1);
  if mr=mrYes then begin
    if not deleteRecord then begin
      ShowError(Application.Handle,'Бланк <'+Mainqr.FieldByName('series').AsString+'> используется.');
    end;
  end;
end;
...
var
  Field: TField;
begin
  Field:=Mainqr.FindField('num_from');
