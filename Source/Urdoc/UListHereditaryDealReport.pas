    ...
    try
      ...
      for i:=0 to qr.Fields.Count-1 do begin
        if qr.Fields[i].Visible then begin
           ...
           if i=0 then begin
             qr.Fields[i].DisplayLabel:='Название наследственного дела';
             qr.Fields[i].DisplayWidth:=280;
           end;  
           if i=1 then begin
             qr.Fields[i].DisplayLabel:='Дата';
             qr.Fields[i].DisplayWidth:=70;
           end;
           if i=2 then begin
             qr.Fields[i].DisplayLabel:='Документ';
             qr.Fields[i].DisplayWidth:=240;
           end;
           if i=3 then begin
             qr.Fields[i].DisplayLabel:='Примечание';
      ...
      while not qr.Eof do begin
        ...
        for i:=-1 to List.Count-1 do begin
          if i=-1 then begin
            ...
            if i=0 then
              SetPositonAndText(Round(Counter),VarToStrDef(NewValue,''),'Название н/д: ',
