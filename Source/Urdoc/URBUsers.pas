...
//   cl.FieldName:='flagadmin';
   cl.Title.Caption:='Администратор';
   cl.Width:=85;
...
//  but:=MessageBox(Handle,Pchar('Удалить ?'),'Предупреждение',MB_YESNO+MB_ICONWARNING);
  if mr=mrYes then begin
    if not deleteRecord then begin
      ShowError(Application.Handle,'Пользователь <'+Mainqr.FieldByName('name').AsString+'> используется.');
    end;
  end;
end;
...
begin
  if not Mainqr.Active then exit;
  if Mainqr.RecordCount=0 then exit;
  if column.Title.Caption='Администратор' then begin
    DrawAdmin;
  end;
end;
