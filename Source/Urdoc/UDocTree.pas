...
var
  mr: TModalResult;
begin
  mr:=MessageDlg('Будут удалены все шаблоны.'+#13+
                               'Удалить все ?',mtConfirmation,[mbYes,mbNo],-1);
  ...
  case mr of
    mrYes: begin
      if not isLinkOnReestrNew then begin
       ...
       ShowError(Application.Handle,'На один из шаблонов есть ссылка в реестре.');
      end;
    end;
  end;
end;
...
begin
  ...
  if Trim(S)='' then begin
   ShowError(Application.Handle,DefaultNodeText+' не может быть пустым.');
   S:=Node.Text;
   exit;
  end;
...
{  TypeDocId:=GetTypeDocID(S);
  if TypeDocId<>0 then begin
   ShowError(Application.Handle,'Такой вид уже существует.');
   S:=Node.Text;
   exit;
  end;}
  P:=Node.data;
...
begin
  nd:=TV.Selected;
  if nd=nil then exit;
  mr:=MessageDlg('Будут удалены все шаблоны из этого вида.'+#13+
                               'Удалить <'+nd.text+'> ?',mtConfirmation,[mbYes,mbNo],-1);

  case mr of
    mrYes: begin
      if nd.Count<>0 then begin
        ShowError(Application.Handle,'На вид <'+nd.Text+'> ссылаются другие виды.');
      end else begin
        DeleteNode(nd);
      end;
    end;
...
begin
  ...
  S:=Trim(S);
  if Trim(S)='' then begin
   ShowError(Application.Handle,'Наименование шаблона не может быть пустым.');
   S:=Item.Caption;
   exit;
  end;
  PList:=Item.data;
...
begin
  ...
  li:=LV.Selected;
  if li=nil then exit;
  mr:=MessageDlg('Удалить выделенные шаблоны?',mtConfirmation,[mbYes,mbNo],-1);
   ...
   if Li.Selected then begin
    ...
    if not isLinkOnReestr(PList.ID,0,true) then begin
     ...
     dispose(Plist);
      ...
      ShowError(Application.Handle,'На шаблон <'+li.caption+'> есть ссылка в реестре.');
      exit;
    end;
   end;
  end;
...
begin
  ...
  mr:=MessageDlg('Удалить все шаблоны в виде <'+nd.Text+'> ?',mtConfirmation,[mbYes,mbNo],-1);
  case mr of
    mrYes: begin
     if not isLinkOnReestr(0,P.Id,false) then begin
...
begin
  ...
  if DirectoryExists(Last) then
   LoadDocFromDir('Добавление...',Last,false);
end;
...
begin
  Last:=GetDir(Application.Handle,'');
  if DirectoryExists(Last) then
   LoadDocFromDir('Загрузка...',Last,true);
end;
...
procedure TfmDocTree.miCopyDocClick(Sender: TObject);
  ...
  begin
    ...
    fmProgress.Caption:='Копирование...';
...
begin
  ...
  try
    ...
    if na_id=0 then
      ShowInfo('Нет привязки к нотариальному действию.');
...
begin
  ...
  try
    ...
    if r_id=0 then
      ShowInfo('Нет привязки к обновлению.');
    fm.BorderIcons:=fm.BorderIcons-[biMinimize];
    fm.bibOk.Visible:=true;
    fm.bibOk.OnClick:=fm.MR;
    fm.Grid.OnDblClick:=fm.MR;
...
begin
  ...
  if Assigned(PCur) then begin
    ...
    try
      fm.Caption:='Установите порядок';
      fm.lbNumber.Caption:='Порядок шаблона:';
...
begin
  ...
  if Assigned(PCur) then begin
    fm:=TfmInputSortNum.Create(nil);
    try
      fm.Caption:='Установите сумму';
      fm.lbNumber.Caption:='Сумма шаблона:';
