...
//  but:=MessageBox(Handle,Pchar('Удалить ?'),'Предупреждение',MB_YESNO+MB_ICONWARNING);
  if mr=mrYes then begin
    if not deleteRecord then begin
      ShowError(Application.Handle,'Правило с именем поля <'+Mainqr.FieldByName('name').AsString+'> используется.');
    end;
  end;
end;
...
procedure TfmRBRuleForElement.GridDrawColumnCell(Sender: TObject;
  ...
  begin
    ...
    if (val>=0) or (val<=ListClassesForWord.Count-1) then begin
      ...
      tmps:='Элемент не поддерживается';
    end;
...
{  if not Mainqr.Active then exit;
  if Mainqr.RecordCount=0 then exit;
  if column.Title.Caption='Тип элемента' then begin
    DrawPlusElementType;
  end;}
end;
