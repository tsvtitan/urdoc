...
begin
  if Trim(edFromNumber.text)<>'' then
    if not isInteger(Trim(edFromNumber.text)) then begin
      ShowError(Handle,'Значение должно быть целым числом.');
  ...
  if (Trim(edFromNumber.text)<>'')and
    ...
    if StrToInt(Trim(edFromNumber.text))>StrToInt(Trim(edToNumber.text)) then begin
      ShowError(Handle,'Номер <с которого> должен быть меньше номера <по>.');
      edFromNumber.SetFocus;
      exit;
    end;
  end;
...
//    SummAll: String;
 ...
 begin
  ...
  try
    ...
    tb.Cell(1,1).Range.InsertBefore('Статистический отчет');
...
//    tb.Cell(2,1).Range.InsertBefore('за период '+periodStr);
    ...
    tb.Cell(4,1).Range.InsertBefore('Количество нотариальных действий');
    Application.ProcessMessages;
    if BreakAnyProgress then exit;
    newValue:=newValue+incr;
    SetPositonAndText(Round(newValue),'','Создание заголовка',nil,fmProgress.gag.Max);
...
//       ctnDefect:=ctnDefect+curCount;
      ...
      SetPositonAndText(Round(newValue),Mainqr.FieldByName('name').AsString,
                        'Нот-е действие: ',nil,fmProgress.gag.Max);
      MainQr.Next;
      inc(i);
    end;
    Application.ProcessMessages;
    if BreakAnyProgress then exit;
    newValue:=newValue+incr;
    SetPositonAndText(Round(newValue),'','Подсчет количества',nil,fmProgress.gag.Max);

    Last:=Last+i+1;
    tb.Rows.Add;
    tb.Cell(last,1).Range.Font.Bold:=true;
    tb.Cell(Last,1).Range.InsertBefore('ИТОГО (гр.'+inttostr(fGroupNum)+'-'
    ...
    tb.Cell(last,1).Range.InsertBefore('Финансовая деятельность');
    tb.Rows.Add;
    Application.ProcessMessages;
    if BreakAnyProgress then exit;
    newValue:=newValue+incr;
   ...
   // tb.Cell(last,1).Merge(tb.Cell(last,2));
    ...
    tb.Cell(last,1).Range.InsertBefore('Наименование');
    tb.Cell(last,2).Range.Paragraphs.Alignment:=wdAlignParagraphCenter;
    tb.Cell(last,2).Range.InsertBefore('Значение');
    Application.ProcessMessages;
    if BreakAnyProgress then exit;
    newValue:=newValue+incr;
    SetPositonAndText(Round(newValue),'','Создание заголовка',nil,fmProgress.gag.Max);
    ...
    tb.Cell(last,1).Range.Font.Bold:=false;
    tb.Cell(last,1).Range.Paragraphs.Alignment:=wdAlignParagraphLeft;
    tb.Cell(last,1).Range.InsertBefore('Всего взыскано тарифа');
    tb.Cell(last,2).Range.Font.Bold:=false;
    tb.Cell(last,2).Range.Paragraphs.Alignment:=wdAlignParagraphRight;
    ...
    Last:=Last+1;
    tb.Rows.Add;
    tb.Cell(last,1).Range.InsertBefore('Стоимость имущества перешедших гос-ву по праву наследования');
    ...
    Last:=Last+1;
    tb.Rows.Add;
    tb.Cell(last,1).Range.InsertBefore('Кол-во нот.действий совершенных по льготному тарифу');
    ...
    Last:=Last+1;
    tb.Rows.Add;
    tb.Cell(last,1).Range.InsertBefore('Размер недополученных средств в следствии применения льгот');
